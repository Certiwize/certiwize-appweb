...
// api/complete-gocardless-checkout.js
async function onRequestPost3(context) {
  ...
  try {
    ...
    if (!completeRes.ok) {
      const rawText2 = await completeRes.text();
      console.error("[complete-gocardless-checkout] Error response text:", rawText2);
      let errData;
      try {
        errData = JSON.parse(rawText2);
      } catch {
        errData = { message: rawText2 };
      }
      console.error("GoCardless complete flow error:", errData);
      return new Response(
        JSON.stringify({ error: "\xC9chec de la validation GoCardless", details: errData }),
    ...
    if (!subscriptionRes.ok) {
      const rawText2 = await subscriptionRes.text();
      console.error("[complete-gocardless-checkout] Subscription error response text:", rawText2);
      let errData;
      try {
        errData = JSON.parse(rawText2);
      } catch {
      ...
      return new Response(
        JSON.stringify({ error: "\xC9chec de la cr\xE9ation de l'abonnement GoCardless", details: errData }),
    ...
    if (!planId) {
      return new Response(
        JSON.stringify({ error: `Plan "${plan}" introuvable en base de donn\xE9es` }),
    ...
    if (existingUser) {
      ...
      if (!createUserRes.ok) {
        const rawText2 = await createUserRes.text();
        console.error("[complete-gocardless-checkout] Create user error response text:", rawText2);
        let errData;
        try {
          errData = JSON.parse(rawText2);
        } catch {
        ...
        return new Response(
          JSON.stringify({ error: "\xC9chec de la cr\xE9ation du compte utilisateur", details: errData }),
          { status: 500, headers: corsHeaders }
        );
      }
    ...
    const endsAt = plan === "yearly" ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1e3).toISOString() : new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString();
    ...
    if (!subUpsertRes.ok) {
      const rawText2 = await subUpsertRes.text();
      console.error("[complete-gocardless-checkout] Subscription upsert error response text:", rawText2);
      let errData;
      try {
        errData = JSON.parse(rawText2);
      } catch {
    ...
    if (!magicLinkRes.ok) {
      const rawText2 = await magicLinkRes.text();
      console.error("[complete-gocardless-checkout] Magic link error response text:", rawText2);
      let errData;
      try {
        errData = JSON.parse(rawText2);
      } catch {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: "Abonnement cr\xE9\xE9. Connectez-vous manuellement."
        }),
    ...
    const magicLinkUrl = magicLinkData.hashed_token ? `${supabaseUrl}/auth/v1/verify?token=${magicLinkData.hashed_token}&type=magiclink&redirect_to=${encodeURIComponent(`${appUrl}/dashboard`)}` : magicLinkData.action_link;
    return new Response(
      JSON.stringify({
        success: true,
        auto_login: true,
...
// api/create-gocardless-subscription.js
async function onRequestPost4(context) {
  ...
  if (!accessToken) {
    return new Response(
      JSON.stringify({ error: "GoCardless non configur\xE9" }),
      { status: 500, headers: corsHeaders }
    );
  }
  const gcBaseUrl = environment === "live" ? "https://api.gocardless.com" : "https://api-sandbox.gocardless.com";
...
async function onRequestPost5(context) {
  ...
  try {
    ...
    if (!insertRes.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "\xC9chec de l'enregistrement", details: errData }),
        { status: 500, headers: corsHeaders }
      );
    }
    return new Response(
...
// ../.wrangler/tmp/pages-HYkgaC/functionsRoutes-0.3122871648760799.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-ImL0In/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-ImL0In/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.19304919917116936.mjs.map
